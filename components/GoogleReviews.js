...
function StarRating({ rating }) {
    return (
        <div className="flex gap-0.5">
            {[1, 2, 3, 4, 5].map((star) => (
                <svg key={star} className="w-4 h-4" viewBox="0 0 20 20" fill={star <= rating ? "#FBBC04" : "#e8e8e8"}>
...
function Avatar({ author, avatar }) {
    ...
    if (avatar) {
        return <img src={avatar} alt={author} className="w-10 h-10 rounded-full object-cover" />;
    }

    return (
        <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold text-sm" style={{ backgroundColor: colors[colorIndex] }}>
            {author.charAt(0)}
        </div>
    );
}
...
function ReviewCard({ review }) {
    return (
        <div className="flex-shrink-0 w-[300px] sm:w-[340px] bg-white rounded-xl border border-sky-100 p-5">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col lg:flex-row gap-8">
                    <div className="lg:w-[240px] flex-shrink-0">
                        <div className="lg:sticky lg:top-24 text-center lg:text-left">
                            ...
                            <div className="flex items-center justify-center lg:justify-start gap-2 mb-1">
                                <span className="text-2xl font-bold text-primary-dark">{reviews.rating}</span>
                                <StarRating rating={Math.round(reviews.rating)} />
                            </div>
                            <p className="text-sm text-text-light mb-3">Based on {reviews.totalReviews} reviews</p>
                    ...
                    <div className="flex-1 min-w-0 relative">
                        {canScrollLeft && (
                            <button onClick={() => scroll("left")} className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-10 w-9 h-9 rounded-full bg-white border border-gray-200 items-center justify-center cursor-pointer" aria-label="Previous reviews">
                                <svg className="w-4 h-4 text-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
                            </button>
                        )}
                        {canScrollRight && (
                            <button onClick={() => scroll("right")} className="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-10 w-9 h-9 rounded-full bg-white border border-gray-200 items-center justify-center cursor-pointer" aria-label="Next reviews">
